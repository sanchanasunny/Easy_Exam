...
def val(image,eid):
	...
	for f in faces:
		...
		# i = i + 1
		cv2.rectangle(imag, (x,y), (x+w,y+h), (0,255,0), 4)

		#Save just the rectangle faces in SubRecFaces
		sub_face = imag[y:y+h, x:x+w]

		FaceFileName = "static/test.jpg" #Saving the current image from the webcam for testing.
		cv2.imwrite(FaceFileName, sub_face)
		# break
		val=rec_face_image(FaceFileName,eid)
		print(val)
		# print("user",val)
		str1=""
		for ele in val:  
...
def rec_face_image(imagepath,eid):
	print(imagepath)




	data = pickle.loads(open('faces'+str(eid)+'.pickles', "rb").read())
...
# corresponding to each face in the input image
		...
		# compute the facial embedding for the face
				encodings = face_recognition.face_encodings(rgb, boxes)
				print(encodings)
				q="insert into facialdetails values(null,'%s','%s','%s')" %(tid,eid,encodings)
				id=insert(q)
				...
				q="update applications set qrimage='%s' where exam_id='%s' and candidate_id='%s'" %(path,eid,tid)
				update(q)
		...
		f = open('faces'+str(eid)+'.pickles', "wb")
